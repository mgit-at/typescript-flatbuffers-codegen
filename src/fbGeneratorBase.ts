...
import {Options} from './fbGenerator';
...
export abstract class FbGeneratorBase {
    ...
    protected readonly n = {
        ...
        proxyHelperVar: (v: string) => `__${v}ProxyHelper`,
        offsetVar: (v: string) => `__of${camelCase(v, true)}`,
        vecPosVar: (v: string) => `__vecPos${camelCase(v, true)}`,
        lengthVar: (v: string) => `__len${camelCase(v, true)}`,
    ...
    protected constructor(schema: fbR.Schema, options: Options) {
        this.schema = schema;
        this.hasProxy = false;
    ...
    protected sanitizeSchema() {
        this.schema.objects.forEach((v) => {
            ...
            const valid = checkReserved(name);
            if (!valid) {
                if (!this.options.postfixReserved) {
                    throw `Invalid name or reserved keyword '${name}'`;
                }
            ...
            v.fields.forEach((f) => {
                ...
                if (f.name.startsWith('__')) {
                    throw `Field-names cannot start with '__': '${f.name}'`;
                }
